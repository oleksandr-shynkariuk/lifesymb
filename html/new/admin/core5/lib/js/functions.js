...
jQuery('a.nts-slot-link').live( 'click', function() {
	jQuery('.nts-ajax-container').html('');
	jQuery('.nts-ajax-container').hide();
	});
...
jQuery('.nts-ajax-container form').live( 'submit', function(event) {
	...
	resultDiv.addClass( 'nts-loading' );
	resultDiv.append( '<div class="nts-load-indicator"><i class="icon-spinner"></i> loading</div>' );
	...
	jQuery.ajax({
		...
		success: function(msg){
			resultDiv.removeClass( 'nts-loading' );
			resultDiv.html( msg );
	       }
	    });
	return false;
...
jQuery('.nts-ajax-container a:not(.nts-ajax-loader)').live( 'click', function(event) {
...
/* else search in children */
		...
		var targetDiv = myParent.find('.nts-ajax-container');
		}

	if( (myTitle == '[+]') || (myTitle == '[-]') ){
		var currentHtml = targetDiv.html();
		if( currentHtml.length ){ // hide
			targetDiv.html('');
			if( myTitle == '[-]' )
				jQuery(this).html('[+]');
			}
...
//			targetDiv.removeClass( 'nts-ajax-selected' );
			}
		else {
			targetDiv.show();
			targetDiv.addClass( 'nts-loading' );
			targetDiv.append( '<div class="nts-load-indicator2"><i class="icon-spinner"></i> loading</div>' );
			targetDiv.data( 'targetUrl', targetUrl );
			targetDiv.load( targetUrl, function(){
				targetDiv.removeClass( 'nts-loading' );
