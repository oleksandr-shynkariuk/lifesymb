...
//		printf("%.2fs", $time );
	...
	static function calcTax( $amount, $taxRate )
	{
		$return = round( ($taxRate * $amount)/100, 2 );
		return $return;
	}

	static function removeTax( $amount, $taxRate )
	{
	...
	static function getMyProduct(){
		$myProduct = 'ha';
		if( NTS_APP_LEVEL == 'solo' ){
			$myProduct = 'has';
			}
	...
	static function generateRand( $len, $conf = array() ){
	...
	static function requireMenuFile( $menuFile, $link2panel = '' ){
		...
		if( ! isset($MENU_FILES[$menuFile]) ){
			...
			$MENU_FILES[$menuFile] = array( $title, $sequence, $params, $directLink, $alert, $permissionsFor );
			}
...
//		header("Content-Length: $fileSize");
	...
	static function listFiles( $dirName, $extension = '' ){
		if( ! is_array($dirName) )
			$dirName = array( $dirName );
