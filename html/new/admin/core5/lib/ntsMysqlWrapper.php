...
EOT;
	...
	function count( $from, $whereArray = array() ){
		...
		$from = array_map( create_function('$a', 'return "{PRFX}$a";'), $from );
		$from = join( ',', $from );
...
class dbWrapper extends ntsMysqlWrapper {
	function dbWrapper(){
		$tbPrefix = NTS_DB_TABLES_PREFIX . 'v5_';
		parent::ntsMysqlWrapper( NTS_DB_HOST, NTS_DB_USER, NTS_DB_PASS, NTS_DB_NAME, $tbPrefix );
		$this->init();
		$this->_debug = false;
		$this->_enableCache = false;
		}
