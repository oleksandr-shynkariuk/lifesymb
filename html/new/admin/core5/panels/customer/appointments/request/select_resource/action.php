...
foreach( $currentIndexes as $i ){
	if( ! count($allValidIds[$i]) ){
		ntsView::setAnnounce( M('Bookable Resource') . ': ' . M('Not Available'), 'error' );
...
foreach( $currentIndexes as $i ){
	...
	if( (count($allValidIds[$i]) == 1) || ($NTS_VIEW['selectionMode'] == 'auto') ){
		...
		$NTS_AR->setSelected( $i, 'resource', $validId );
...
if( $redirect ){
	...
	if( $tryChange ){
		ntsView::addAnnounce( M('Bookable Resource') . ': ' . M('No more options available'), 'error' );
		$noForward = false;
		}
	else {
		$noForward = true;
...
foreach( $currentIndexes as $i ){
	...
	foreach( $allValidIds[$i] as $vid ){
		$validOne = ntsObjectFactory::get( 'resource' );
		$validOne->setId( $vid );
		$entries[$i][] = $validOne;
		}
	/* sort by show order */
