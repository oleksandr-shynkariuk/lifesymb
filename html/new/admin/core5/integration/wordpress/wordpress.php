...
WHERE
	...
	obj_id NOT IN ( SELECT $theidIdIndex FROM {PRFX}users )
EOT;
	...
	function queryUsers( $where = array(), $order = array(), $limit = '' ){
		...
		if( isset($where['first_name']) ){
			$where['(SELECT meta_value FROM {PRFX}usermeta WHERE meta_key="first_name" AND user_id={PRFX}users.ID)'] = array( $where['first_name'][0], $where['first_name'][1] );
			unset( $where['first_name'] );
			}

		if( isset($where['last_name']) ){
			$where['(SELECT meta_value FROM {PRFX}usermeta WHERE meta_key="last_name" AND user_id={PRFX}users.ID)'] = array( $where['last_name'][0], $where['last_name'][1] );
		...
		$result = $this->db->select( 'ID', 'users', $where, $orderString . ' ' . $limitString );
		if( $result ){
		...
		$count = $this->db->count( 'users', $where );
